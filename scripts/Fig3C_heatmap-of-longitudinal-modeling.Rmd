...
## Data Processing
```{r}
processed.data <- read.csv(file.path("..", 'data', "2023-12-01_tumor-size_R-formatted.csv")) %>%
...
mimic.modeling %>%
  ...
  mutate(term.adj = fct_relevel(term.adj,"Anti-PD1:Gavage","Anti-PD1","Gavage")) %>%
...
ggsave("../figures/Fig3C_heatmap-of-longitudinal-modeling.png",
       height =  6, width = 6)
ggsave("../figures/Fig3C_heatmap-of-longitudinal-modeling.svg",
       height =  6, width = 6)
```

```{r}
mimic.modeling1 <- mimic.modeling %>%
  mutate(`Combined ID` = case_when(
    `Combined ID` == "Non-responder (DL081)" ~ "Non-responder",
    `Combined ID` == "Responder (DL017)" ~ "Responder",
    TRUE ~ `Combined ID`
  ))

mimic.modeling1 <- mimic.modeling1 %>%
  mutate(term.adj = gsub("days2:(.*)", "\\1", term)) %>%
  mutate(term.adj = gsub("treatmentPD1", "treatmentAnti-PD1", term)) %>%
  filter(!grepl("mimic4|mimic9", experiment)) %>%
  filter(grepl("days2:", term)) %>%
...
mimic.modeling1 <- mimic.modeling1 %>%
  mutate(Group = case_when(
    `Combined ID` %in% c("Non-responder", "Responder") ~ "ICI Controls",
    `Combined ID` %in% c("Random (102)", "Random (55)") ~ "Random",
    `Combined ID` %in% c("Blautia (84)", "Blautia (85)", "Roseburia (68)", "Lachnospira (79)") ~ "Lachnospiraceae",
    TRUE ~ "Other" # For any cases not matched above
  )) %>%
  mutate(Group = factor(Group, levels = c("ICI Controls", "Random", "Lachnospiraceae"))) %>%
  rename(`Effect Size` = estimate) 

mimic.modeling1 %>%
  filter(Group != "ICI Controls") %>%
  ggplot(aes(x = `Combined ID`, y = term.adj, fill = `Effect Size`)) +
  geom_tile(aes(alpha=sig.flag),show.legend = T) +
  scale_fill_viridis_c() +
  facet_wrap(~ Group, ncol = 3, scales= "free_x", strip.position = "top") +
...
ggsave("../figures/Fig3C_heatmap-of-longitudinal-modeling1_grant.png",
       height =  2, width = 4)

ggsave("../figures/Fig3C_heatmap-of-longitudinal-modeling1.png",
       height =  6, width = 6)
ggsave("../figures/Fig3C_heatmap-of-longitudinal-modeling1.svg",
       height =  6, width = 6)
...
mimic.modeling %>%
  ...
  mutate(`Combined ID` = fct_relevel(`Combined ID`,
                                     ...
                                     "Roseburia (68)",
                                     "Lachnospira (79)"),
         fac = ifelse(grepl("Random", hypothesis), "Random",
                        ifelse(grepl("Roseburia|Lachnospira|Blautia", hypothesis), "Lachnospiraceae", NA))) %>%
  filter(!is.na(fac)) %>%
  ggplot(aes(x = `Combined ID`, y = term.adj, fill = estimate, alpha = sig.flag)) +
  geom_tile(color = "black") +
  scale_fill_viridis_c() +
